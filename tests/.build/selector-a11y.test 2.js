/* selector-a11y.test.svelte.js generated by Svelte v5.56.8 */
...
function mountOne(Comp, props) {
	...
	return {
		target,
		st,
		group: () => target.querySelector('[role="radiogroup"]'),
...
for (const [name, Comp, props] of [
	...
	ok(`${name}: every option has role=radio`, opts.every((o) => o.getAttribute('role') === 'radio'), opts.map((o) => o.getAttribute('role')).join(','));

	const checked = opts.map((o) => o.getAttribute('aria-checked'));

	eq(`${name}: aria-checked marks exactly the selected one`, checked.join(','), 'false,true,false');
	...
	eq(`${name}: aria-checked follows the click`, m.opts().map((o) => o.getAttribute('aria-checked')).join(','), 'false,false,true');
	m.target.remove();
}
...
// ---- options are text unless the consumer asks for markup ----
{
	...
	mount(Select, {
		target,
		props: {
			options: ['a', '<img src=x onerror=alert(1)>b'],
			get value() {
				return st.v;
			},
