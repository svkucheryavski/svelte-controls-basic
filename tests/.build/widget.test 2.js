/* widget.test.svelte.js generated by Svelte v5.56.8 */
...
function makeWidget(options, initial) {
	...
	const state = $.proxy({ value: initial });
	...
	const labels = () => [...target.querySelectorAll('.control-element > span.label')].map((e) => e.textContent.trim());
...
// ----------------------------- end to end: change the real Select element
{
	...
	const btn = [...w.target.querySelectorAll('.selector button.option')].find((b) => b.textContent.trim() === 'both');
...
// ---- a validation message must survive the control being hidden and shown again ----
{
	...
	const w = makeWidget({
		mode: {
			label: 'mode',
			el: Select,
			props: { options: ['on', 'off'] },
			default: 'on'
		},
		name: {
			label: 'name',
			el: TextInput,
			props: { validator: short },
			default: '',
	...
	ok('field still marked invalid', !!w.target.querySelector('.textinput.error'), w.target.querySelector('.textinput')?.className);
	w.done();
}
