...
>
   ...
   <!-- 'aria-checked' and not the class alone: a class paints the button, it does not tell a
        screen reader which option is the chosen one. These buttons hold an icon and no text,
        so they also need a name of their own -->
   <button type="button" role="radio" aria-checked={option === value} aria-label={titleFor(option) ?? option} tabindex="-1" disabled={disable} onclick={() => selectOption(option)} class="option" title={titleFor(option)} class:selected={option===value}>
